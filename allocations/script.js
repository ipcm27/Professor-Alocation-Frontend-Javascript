...
async function createLine(allocation) {
  ...
  const infoHorario = `${allocation.start.substr(
    0,
    5
  )} - ${allocation.end.substr(0, 5)}`;
  ...
  myInputs.forEach(function (input) {
    input.addEventListener("blur", function () {
      if (
        ...
        end_input.value != "selected"
      ) {
        document.getElementById("selectStartHourId").style.borderColor =
          "#e5e9ec";
        document.getElementById("selectEndHourId").style.borderColor =
          "#e5e9ec";
...
function iconDelete_click(allocation) {
  ...
  txtAllocation.textContent = `${allocation.professor.name} - ${
    allocation.course.name
  }: ${allocation.start.substr(0, 5)} - ${allocation.end.substr(0, 5)} `;
  document.getElementById("msgConfirmDelete").hidden = false;
  document.getElementById("msgConfirmDeleteAll").hidden = true;

  var myModalDelete = new bootstrap.Modal(
